     ...
     class PersonaDAO extends ConBdMysql {
        ...
        public function actualizar() {
         try {
            ...
            $estado = $registro[0]['perEstado'];
            $created = $registro[0]['per_created_at'];
            $updated = $registro[0]['per_updated_at'];
            $usuario = $registro[0]['usuario_s_usuId'];
            $id = $registro[0]['perId'];
            if (isset($id)) {
               $actualizar = "UPDATE persona SET  perDocumento=?,perNombre=?,perApellido=?,perEstado=?,per_created_at=?,per_updated_at=?,usuario_s_usuId=? WHERE perId=?;";
               $actualizacion =$this->prepare($actualizar);
               $resultadoAct = $actualizacion -> execute(array($documento,$nombre, $apellido, $estado,  $created, $updated,$usuario,$id));
               $this->cierreBd();
